...
public class BusinessProductDAO {
	private int productAmount;

	public BusinessProductDAO() {
		super();
	}
	...
	public int getProductAmount(int currentID) {
		...
		try{
			...
			String hql="from ProductInfo where businessId ="+currentID;
	...
	public int getProductAmount(ProductInfo model, int currentID) {
		...
		try{
			...
			StringBuilder hql=new StringBuilder("from ProductInfo where 1=1 and businessId="+currentID);
	...
	public List<ProductInfo> findAll(int currentID) {
		try{
			Session session = HibernateUtils.openSession();
			Transaction tx = session.beginTransaction();
